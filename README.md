...
| `grep -c 'server.tool(' index.js` | Confirm the tool count (should be 135) |
| `npm publish` | Publish to npm — **this is deployment** (requires 2FA passkey from an interactive terminal) |

## Tools

All 135 tools, grouped as they appear in `index.js`. ⚠️ marks destructive tools; their descriptions state the blast radius.
...
| Path | Role |
|------|------|
| `index.js` | The whole server: `--setup` flow, config read, `api()` HTTP helper, `text()`/`body()` helpers, all 135 `server.tool(...)` registrations, transport connect. |
